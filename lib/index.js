 ...
 * @param {Function} fn Callback function on end
 * @return {null}
 */
function send(track, fn) {
  ...
  var getName = function() {
    var name = track.proxy('context.traits.name');
    if (name) return name;

    var firstName = track.proxy('context.traits.firstName');
    var lastName = track.proxy('context.traits.lastName');
    if (firstName && lastName) return firstName + ' ' + lastName;
    if (firstName) return firstName;
    if (lastName) return lastName;

    var username = track.proxy('context.traits.username');
