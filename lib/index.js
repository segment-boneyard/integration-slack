...
Slack.ensure(function(msg, settings) {
  if (msg.type() === 'identify') {
    if (!includesKeys(identify.traits(), settings.whiteListedTraits)) return this.reject('Only identify calls that include `traits` in settings will be sent.');
  }
});
...
function identify(identify, fn) {
  var template = this.settings.identifyTemplates['identify'] || 'Identified user, {{name}} at {{email}}. \n{{traits}}';
