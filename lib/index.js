...
function send(track, fn){
  ...
  var getName = function() {
    ...
    return track.proxy('traits.username') || track.email() || 'User ' + track.userId();
  };

  var template = this.settings.templates[track.event()] || '{{name}} did {{event}}.';
  var compiled = handlebars.compile(template);
  var text = compiled(extend(track, {
    name: getName()
  }));

  var payload = { text: text };
  payload.username = "Segment";
  var channel = this.settings.channels[track.event()];
  if (channel) payload.channel = channel;
  payload.icon_url = 'https://logo.clearbit.com/segment.com';
